# État actuel

> Ce fichier est injecté au démarrage de chaque session. Il ne contient que ce qui est **vivant maintenant**.
> Tout ce qui est résolu en sort et reste dans `memory.md`. Cible : rester sous 4 000 caractères.
> Dernière mise à jour : **24/08/2026**

## ⏰ Échéances

| Quand | Quoi | État |
|---|---|---|
| **16-17 sept.** | Formation DDA Radiance, agence de Dijon, via MyConnecting | Supports prêts, 2 trous de contenu |
| **avant le 31/08** | Point de cadrage Radiance | ✉️ **Relancé le 24/08** avec les 7 questions et les supports en pièces jointes — en attente de réponse |
| **cette semaine** | Devoir donné par Elisabeth le 24/08 : les croyances qui m'empêchent, et ma plus grande peur | à faire |
| **dimanche 18h** | 1ère exécution auto du cron notes iOS — vérifier le log lundi | à surveiller |

## 🔨 Chantiers actifs

**MyConnecting / Radiance.** Déroulé HTML v6, déroulé pédagogique 15 pages, supports J1 (43 slides) et J2 (51 slides). Restent deux trous que seul le client peut boucher : la conséquence factuelle d'une formule au minimum sur un deux-roues, et la répartition des six informations dues. Et le texte de garantie du test d'entrée (J1, 9h45) est une clause inventée : elle est **partie chez le client le 24/08 sans mention visible**. Un bandeau d'avertissement a été ajouté sur la slide depuis ; à signaler à Maori, ou à évoquer au cadrage.

**Cercle de Vérité.** Produit complet et prêt. Test Meta à 200 € **pas encore lancé**. Le prix de l'upsell 1 n'est toujours pas arrêté (17/27/47/57 € tous tournés). Question de fond ouverte depuis le 08/08 : par quoi remplacer les 3 besoins que la sur-adaptation satisfait ? Piste depuis le 24/08 → la matière du coaching Elisabeth Roche.

**Sakura.** Architecture cible à deux niveaux (mid ~997 €, high ~2 500 €) **décidée mais pas construite** : ni contenus différenciés, ni pages, ni mécanique de passage. Le taux d'upgrade nécessaire depuis le Cercle de Vérité est à recalculer une fois les niveaux définis.

**Coaching Elisabeth Roche.** Hebdomadaire. Trois séances tenues. Prochaine annoncée « plus technique : carré, discipline, ressources ».

**Contenu Mnemonaute.** Backlog écrit, rien lancé. Priorité 1 : la série « enchaîner les méthodes », dont la vidéo Code de la route 2018 est déjà la démonstration complète. Quick win jamais fait : ajouter le CTA du lead magnet aux 5 vidéos les plus vues (1,18 M de vues, une après-midi de travail).

## ⏸️ En attente d'une décision de David

- **Les deux livres.** *Libérez votre mémoire* en jachère depuis le 16/08 (préface Olicard marquée comme accordée). *Le Triangle Dramatique* en rédaction. Aucun n'avance. Lequel passe devant ?
- **La tension marketing.** Elisabeth dit « le marketing, on s'en fout, sois toi-même » ; la stratégie en cours, c'est ads Meta → funnel. À trancher consciemment avant le lancement, pas à subir.
- **Trello.** Écarté au profit du plugin Kanban d'Obsidian — sauf besoin de partager un tableau avec un tiers.

## ⚠️ Points de vigilance

- **Secrets en clair.** La note « ADMIN » du dossier iCloud *Level-Up Attitude* contient toujours le token Anthropic, celui du bot Telegram, l'IBAN Qonto et des codes de récupération. Signalé le 20/07, non résolu. Le dossier est exclu en dur de la synchro des notes, mais le problème d'origine reste entier.
- **Administratif MyConnecting**, non clos mais volontairement en pause (décision du 08/08) : attestation URSSAF au nom propre et non de la société, RIB à l'adresse de Craponne, Qualiopi introuvable. **Ne pas relancer de moi-même.**
- **Signaux comportementaux** (voir CLAUDE.md) : procrastination admin, Magic Arena, dispersion, absence de Pomodoro, le petit singe.
